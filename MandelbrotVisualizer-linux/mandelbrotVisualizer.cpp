...
void mandelbrotVisualizer::setScreenSize()
{
    ...
    if(true){
        ...
        if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &w) == 0) {
            width = std::round(w.ws_col * 0.48);    // équivalent à csbi.srWindow.Right - Left + 1
            height = std::round(w.ws_row * 0.92) - 3; // équivalent à csbi.srWindow.Bottom - Top + 1

            if (height % 2 == 1) {
                height--; // La hauteur doit être paire
            }
        } else {
            // En cas d’échec, valeur par défaut (optionnel)
            width = 80;
            height = 24;
        }
